...
@section('content')
    <div class="flex flex-col gap-2 justify-center align-middle h-screen w-3/12">
        ...
        @endif
        <h3 class="text-lg font-semibold" data-aos="fade-down" data-aos-duration="500" data-aos-easing="ease-in-out">Login Page</h3>
        <form class="flex flex-col gap-2" method="POST" action={{ route('login_action') }}>
            @csrf
            ...
            <button type="submit" class="bg-slate-700 py-2 rounded-md my-2 hover:bg-slate-600 transition ease-in-out duration-300" data-aos="fade-left" data-aos-duration="500" data-aos-easing="ease-in-out" data-aos-offset="100" data-aos-delay="1500">Login</button>
        </form>
        <i data-aos="fade-in" data-aos-duration="500" data-aos-easing="ease-in-out" data-aos-offset="50" data-aos-delay="2000">
            <a href="/register" class="text-sm text-blue-500 hover:border-b transition ease-in-out">not have account</a>
        </i>
    </div>
@endsection
